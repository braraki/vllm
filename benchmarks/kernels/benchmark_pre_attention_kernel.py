...
"""Benchmark the Gemma4 pre-attention kernel against the compiled baseline."""
...
DEFAULT_PROVIDERS = ["baseline_compiled", "pre_attention_kernel_custom_op"]
...
def pre_attention_kernel_custom_op(
    qkv: torch.Tensor,
    positions: torch.Tensor,
    q_weight: torch.Tensor,
    k_weight: torch.Tensor,
...
def benchmark_provider(
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("benchmark_pre_attention_kernel requires CUDA")
    ...
    with set_current_vllm_config(vllm_config), set_forward_context(
        ...
        if provider == "baseline_compiled":
            compiled_fn = torch.compile(baseline_pre_attention)
            fn = lambda: compiled_fn(*args)
        elif provider == "pre_attention_kernel_custom_op":
            fn = lambda: pre_attention_kernel_custom_op(*args)
        else:
...
def validate_outputs(
    ...
    with set_current_vllm_config(vllm_config), set_forward_context(
        ...
        baseline_q, baseline_k, baseline_v, _ = baseline_pre_attention(
            case["qkv"],
            case["positions"],
            case["q_weight"],
            case["k_weight"],
        ...
        fused_q, fused_k, fused_v, _ = pre_attention_kernel_custom_op(
            case["qkv"],
            case["positions"],
            case["q_weight"],
            case["k_weight"],
...
def write_plots(rows: list[dict[str, float | int | str]], output_dir: Path) -> None:
    ...
    for head_dim in head_dims:
        ...
        plt.title(f"Pre-Attention Kernel Benchmark (head_dim={head_dim})")
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(output_dir / f"pre_attention_kernel_head_dim_{head_dim}.png")
        plt.close()


def parse_args():
    parser = FlexibleArgumentParser(
        description="Benchmark Gemma4 pre-attention kernel implementations."
    )
    parser.add_argument(
        "--num-tokens",
        nargs="+",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./benchmark_pre_attention_kernel_results"),
...
if __name__ == "__main__":
    ...
    write_csv(rows, args.output_dir / "pre_attention_kernel_benchmark.csv")
