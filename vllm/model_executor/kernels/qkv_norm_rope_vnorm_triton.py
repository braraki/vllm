...
def _raise_if_unsupported(
    ...
    if not HAS_TRITON:
        raise RuntimeError(
            ...
            "Triton, but Triton is not available"
        )
    if not current_platform.is_cuda():
        raise RuntimeError(
            "fused_qkv_norm_rope_vnorm_and_unified_kv_cache_update Triton path "
            "is CUDA-only"
        )
    if not qkv.is_cuda:
    ...
    if (
        ...
        or not cos_sin_cache.is_cuda
        or not position_ids.is_cuda
    ):
        raise RuntimeError(
            "All fused_qkv_norm_rope_vnorm_and_unified_kv_cache_update inputs "
            "must be CUDA tensors"
        )
    if (
        q_weight.device != qkv.device
        or k_weight.device != qkv.device
        or v_weight.device != qkv.device
    ...
    if q_weight.dim() != 1 or k_weight.dim() != 1 or v_weight.dim() != 1:
        raise RuntimeError("q_weight, k_weight, and v_weight must be 1D tensors")
    if (
        ...
        raise RuntimeError(
            "q_weight, k_weight, and v_weight must be contiguous tensors"
        )
    if (
        q_weight.dtype != qkv.dtype
        or k_weight.dtype != qkv.dtype
        or v_weight.dtype != qkv.dtype
    ):
        raise RuntimeError("q_weight, k_weight, and v_weight must match qkv dtype")
    ...
    if cos_sin_cache.size(1) != head_dim:
        raise RuntimeError(
            "Gemma4 Triton fused_qkv_norm_rope_vnorm_and_unified_kv_cache_update "
            "requires rotary_dim == head_dim"
        )
    if num_heads_q != _SUPPORTED_NUM_HEADS_Q:
        raise RuntimeError(
            "Gemma4 Triton fused_qkv_norm_rope_vnorm_and_unified_kv_cache_update "
            "requires num_heads_q == 8"
        )
    if (
        ...
        raise RuntimeError(
            "Gemma4 Triton fused_qkv_norm_rope_vnorm_and_unified_kv_cache_update "
            "requires num_heads_k == num_heads_v == 1"
        )
    if head_dim not in _SUPPORTED_HEAD_DIMS:
        raise RuntimeError(
            "Unsupported head_dim for "
            "fused_qkv_norm_rope_vnorm_and_unified_kv_cache_update: "
            f"{head_dim}"
        )
    if (
        q_weight.numel() != head_dim
        or k_weight.numel() != head_dim
        or v_weight.numel() != head_dim
    ):
        raise RuntimeError("q_weight, k_weight, and v_weight sizes must match head_dim")
...
def _validate_pre_attention_kernel_support(
    qkv: torch.Tensor,
    num_heads_q: int,
    num_heads_k: int,
    num_heads_v: int,
...
def _fused_qkv_norm_rope_vnorm_and_unified_kv_cache_update_impl(
    ...
    (
        ...
        kv_cache,
        layer_slot_mapping,
    ) = _validate_pre_attention_kernel_support(
        qkv,
        num_heads_q,
        num_heads_k,
        num_heads_v,
