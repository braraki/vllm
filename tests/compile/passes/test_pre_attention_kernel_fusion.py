...
class PreAttentionKernelTestModel(torch.nn.Module):
    def __init__(
        self,
        *,
        num_heads: int,
...
def _make_vllm_config(dtype: torch.dtype) -> VllmConfig:
    return VllmConfig(
...
@pytest.mark.skipif(
    ...
    reason="pre-attention-kernel compile-pass coverage is CUDA-only",
)
def test_pre_attention_kernel_fusion(dtype, head_dim, is_neox, eps):
    ...
    token_count = 5
    slot_mapping = torch.arange(token_count, dtype=torch.long, device="cuda")

    with (
        set_current_vllm_config(vllm_config),
        vllm_config.kernel_config.ir_op_priority.set_priority(),
    ):
        model = PreAttentionKernelTestModel(
            num_heads=8,
            num_kv_heads=1,
            head_dim=head_dim,
            eps=eps,
        ...
        qkv = torch.randn(token_count, model.q_size + 2 * model.kv_size)
        pos = torch.arange(token_count, dtype=torch.long, device=qkv.device)
...
def test_pre_attention_kernel_does_not_match_without_v_norm(dtype):
    if FUSED_QKV_ROPE_VNORM_KVCACHE_OP is None:
        pytest.skip("pre-attention-kernel custom op not available")
...
def test_pre_attention_kernel_skips_extra_q_user(dtype):
    if FUSED_QKV_ROPE_VNORM_KVCACHE_OP is None:
        pytest.skip("pre-attention-kernel custom op not available")
...
def test_pre_attention_kernel_does_not_match_flash_attention(dtype):
    if FUSED_QKV_ROPE_VNORM_KVCACHE_OP is None:
        pytest.skip("pre-attention-kernel custom op not available")
...
def test_pre_attention_kernel_does_not_match_unsupported_signature(dtype):
    if FUSED_QKV_ROPE_VNORM_KVCACHE_OP is None:
        pytest.skip("pre-attention-kernel custom op not available")
