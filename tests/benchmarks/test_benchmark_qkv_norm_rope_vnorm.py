...
from benchmarks.kernels.benchmark_qkv_norm_rope_vnorm import (
    _resolve_signatures,
    benchmark_provider,
    validate_outputs,
    write_csv,
...
def test_write_csv_smoke(tmp_path: Path):
    output_path = tmp_path / "qkv_norm_rope_vnorm_benchmark.csv"
    write_csv(
        [
            {
                "provider": "attention_prep_custom_op",
                "num_tokens": 1,
                "head_dim": 256,
                "num_heads": 8,
                "num_kv_heads": 1,
    ...
    assert "attention_prep_custom_op" in output_path.read_text()
