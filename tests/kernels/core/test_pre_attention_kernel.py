...
@pytest.mark.skipif(
    ...
    reason="pre-attention-kernel requires CUDA",
)
...
def test_pre_attention_kernel_matches_reference(
    dtype: torch.dtype,
    is_neox: bool,
    eps: float,
    num_tokens: int,
    ...
    with set_current_vllm_config(vllm_config), set_forward_context(
        ...
        baseline_q, baseline_k, baseline_v = _baseline_pre_attention(
            qkv,
            positions,
            q_weight,
            k_weight,
        ...
        fused_q, fused_k, fused_v = _fused_pre_attention(
            qkv,
            positions,
            q_weight,
            k_weight,
...
@pytest.mark.parametrize("head_dim", HEAD_DIMS)
@torch.inference_mode()
def test_pre_attention_kernel_skips_missing_slot_mapping(
    dtype: torch.dtype,
    head_dim: int,
):
    if not hasattr(torch.ops, "vllm") or not hasattr(
